/* ==========================================================================
   4. MÓDULO: ÔNIBUS TRANSLADO (GRADE E ALERTAS)
   ========================================================================== */
...
function verificarProximoOnibus() {
    ...
    // Garante que o alerta esteja sempre visível agora
    alerta.style.display = 'flex';

    // 1. VERIFICAÇÃO DE INTERVALOS (Prioridade máxima)
    const intervaloAlmoco = (hora >= 13 && hora < 14);
    const intervaloJanta = (hora >= 20 && hora < 21);

    if (intervaloAlmoco || intervaloJanta) {
        alerta.style.backgroundColor = '#FFD400'; // Amarelo
    ...
    if (proximo) {
        ...
        // SE FALTAR 5 MINUTOS OU MENOS: Mostra o aviso de saída (Verde)
        if (faltam <= 5 && faltam > 0) {
            let trajeto = proximo.orig === "ESTAC." 
                ? "ESTACIONAMENTO ➔ LOJA" 
                : "LOJA ➔ ESTACIONAMENTO";

            alerta.style.backgroundColor = '#34d82e'; // Verde vivo
        ...
        // 3. MENSAGEM PADRÃO (Quando não tem saída próxima)
        else {
            alerta.style.backgroundColor = 'white'; // Azul Ferreira Costa
            alerta.style.color = '#000000'; // Texto Branco
        ...
        // Caso todos os horários do dia já tenham passado
        alerta.style.backgroundColor = '#334155'; // Cinza escuro
